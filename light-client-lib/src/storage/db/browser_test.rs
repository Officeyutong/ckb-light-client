...
impl From<Vec<u8>> for PinnedSlice {
...
#[derive(Clone)]
pub struct Storage {
    pub(crate) db: Arc<RwLock<BTreeMap<Vec<u8>, Vec<u8>>>>,
}

impl Storage {
    pub async fn new<P: AsRef<Path>>(_path: P) -> Self {
    ...
    pub fn snapshot(&self) -> std::sync::RwLockReadGuard<'_, BTreeMap<Vec<u8>, Vec<u8>>> {
        self.db.read().unwrap()
    }
    ...
    pub fn get<K: AsRef<[u8]>>(&self, key: K) -> Result<Option<Vec<u8>>> {
        Ok(self.db.read().unwrap().get(key.as_ref()).cloned())
    }

    fn get_pinned<'a, K>(&'a self, key: K) -> Result<Option<PinnedSlice>>
    where
        K: AsRef<[u8]>,
    {
        Ok(self
    ...
    fn delete<K: AsRef<[u8]>>(&self, key: K) -> Result<()> {
        self.db.write().unwrap().remove(key.as_ref());
        Ok(())
    }
    ...
    pub fn is_filter_scripts_empty(&self) -> bool {
        ...
        self.db
            .read()
            .unwrap()
            .range((Bound::Included(key_prefix.clone()), Bound::Unbounded))
            .take_while(|(key, _value)| key.starts_with(&key_prefix))
            .next()
            .is_none()
    }

    pub async fn get_filter_scripts_async(&self) -> Vec<ScriptStatus> {
        self.get_filter_scripts()
    }
    ...
    pub async fn update_filter_scripts(
        ...
        match command {
            SetScriptsCommand::All => {
                ...
                let remove_keys: Vec<Vec<u8>> = self
                    .db
                    .read()
                    .unwrap()
                    .range((Bound::Included(key_prefix.clone()), Bound::Unbounded))
    ...
    fn get_matched_blocks(&self, direction: Direction) -> Option<(u64, u64, Vec<(Byte32, bool)>)> {
        ...
        match direction {
            Direction::Forward => self
                ...
                .read()
                .unwrap()
                .range((Bound::Included(iter_from.clone()), Bound::Unbounded))
                .take_while(|(key, _value)| key.starts_with(&key_prefix))
                .map(|(key, value)| {
    ...
    pub fn update_block_number(&self, block_number: BlockNumber) {
        ...
        let mut batch = self.batch();
        let update_list: Vec<(Vec<u8>, Vec<u8>)> = self
            .db
            .read()
            .unwrap()
            .range((Bound::Included(key_prefix.clone()), Bound::Unbounded))
            ...
            .filter_map(|(key, value)| {
                ...
                if stored_block_number < block_number {
                    Some((key.clone(), block_number.to_be_bytes().to_vec()))
                } else {
                    None
                }
            })
    ...
    pub fn rollback_to_block(&self, to_number: BlockNumber) {
        ...
        for ss in scripts {
            if ss.block_number >= to_number {
                ...
                let changes_list: Vec<(Vec<u8>, Vec<u8>)> = self
                    .db
                    .read()
                    .unwrap()
...
impl Storage {
    ...
    pub async fn cell(&self, out_point: &OutPoint, _eager_load: bool) -> CellStatus {
        CellProvider::cell(self, out_point, _eager_load)
    }
}
